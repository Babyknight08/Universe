<?php
    ...
    if (isset($_FILES['eccfile'])) {
        ...
        if (move_uploaded_file($_FILES['eccfile']['tmp_name'], $fileTransfer)) {
            if ($referenceno != '') {
                $nRows = $db_con->query("SELECT COUNT(*) FROM tblprojects WHERE ReferenceNo = '".$referenceno."'")->fetchColumn();
                if ($nRows >= 1) {
                    $response = array(
            ...
            $sql = "INSERT INTO tblprojects (PEISS, ReferenceNo, ECCStatus, DateApproved, Region, PSICCode, ProjectName, Proponent, ProjectType, ProjectSubtype, ProjectSpecificType, ProjectSpecificSubtype, SpecificAddress, Municipality, Province, Latitude, Longitude, RA8749, RA9275, RA6969, CMR, EccFile, ForeignKey) VALUES (:PEISS, :ReferenceNo, :ECCStatus, :DateApproved, :Region, :PSICCode, :ProjectName, :Proponent, :ProjectType, :ProjectSubtype, :ProjectSpecificType, :ProjectSpecificSubtype, :SpecificAddress, :Municipality, :Province, :Latitude, :Longitude, :RA8749, :RA9275, :RA6969, :CMR, :EccFile, :ForeignKey)";
            $stmt = $db_con->prepare($sql);
            $stmt->bindParam(':PEISS', $peiss, PDO::PARAM_STR);
            $stmt->bindParam(':ReferenceNo', $referenceno, PDO::PARAM_STR);
            $stmt->bindParam(':ECCStatus', $status, PDO::PARAM_STR);
            ...
            $stmt->bindParam(':RA6969', $tchwcheck, PDO::PARAM_STR);
            $stmt->bindParam(':CMR', $cmrcheck, PDO::PARAM_STR);
            $stmt->bindParam(':EccFile', $fileTransfer, PDO::PARAM_STR);
            $stmt->bindParam(':ForeignKey', $foreignkey, PDO::PARAM_STR);
            $stmt->execute();
