<?php
    ...
    // -----------------------
    // $cmryear = $data->cmryear;
    // $cmrsemester = $data->cmrsemester;
    // -----------------------
    ...
    // $cmryear_len = count($cmryear);
    // $cmrsem_len = count($cmrsemester);

    $fsem = array('BX', 'BZ', 'CB', 'CD', 'CF');
    $ssem = array('BY', 'CA', 'CC', 'CE', 'CG');
    $year_arr = array(2018, 2019, 2020, 2021, 2022);
    $sem_arr = array('First Semester', 'Second Semester');
    ...
    for($z=0;$z<count($year_arr);$z++) {
        $sheet->getColumnDimension($fsem[$z])->setVisible(false);
        $sheet->getColumnDimension($ssem[$z])->setVisible(false);
    }

    $sql = 'SELECT * FROM tblprojects';          //Initial Query

    if($peisscheck==false || $provincecheck==false || $statuscheck==false || $ptypecheck==false || $psubtypecheck==false || $pstypecheck==false || $pssubtypecheck==false){
        $sql .= $where;
        if($peisscheck==false){
            $sql .= 'PEISS="'.$peiss.'"';
            $firstCondition = 0;
        ...
        foreach($result as $row){
            ...
            if($permitcheck == false){  //IF USER IS FILTERING FOR A SPECIFIC PERMIT TYPE
                ...
                if($stmt->rowCount()==1){
                    // CMR COUNTING
                    if($cmrcheck==false){
                        foreach($year_arr as $yearSelected) {
                            $same_line = false; //THIS IS TO PREVENT THE CODE FROM REWRITING EXISTING INFO ON THE SAME ROW IN OUR EXCEL FILE
                            $current_cmr = "y{$yearSelected}";
                            foreach ($data->$current_cmr as $semesterSelected) {
                                if($semesterSelected!=[]) {
                                    // ====================================================================
                                    $sqli = "SELECT * FROM tblcmr WHERE ProjectID=:ProjectID AND YearSubmission=:YearSubmission AND Semester=:Semester";
                                    $stmt=$db_con->prepare($sqli);
                                    $stmt->bindParam(':ProjectID', $projectID, PDO::PARAM_STR);
                                    $stmt->bindParam(':YearSubmission', $yearSelected, PDO::PARAM_STR);
                                    $stmt->bindParam(':Semester', $semesterSelected, PDO::PARAM_STR);
                                    $stmt->execute();
                                    ...
                                    if($stmt->rowCount()==1){
                                        ...
                                        // \\\\\\\\\\SET $same_line VARIABLE TO true\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
                                        $same_line = true;
                                        // \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
                                        // \\\\\\\\\\\UPDATE CMR COLUMNS\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
                                        for($z=0;$z<count($year_arr);$z++) {
                                            if(intval($yearSelected) == $year_arr[$z]) {
                                                if($semesterSelected=='First Semester') {
                                                    $sheet->getColumnDimension($fsem[$z])->setVisible(true);
                                                    $sheet->setCellValue($fsem[$z].$rowNo, $cmrow['DateSubmission']);
                                                }
                                                if($semesterSelected=='Second Semester') {
                                                    $sheet->getColumnDimension($ssem[$z])->setVisible(true);
                                                    $sheet->setCellValue($ssem[$z].$rowNo, $cmrow['DateSubmission']);
                                                }
                                            }
                                        }
                        ...
                        foreach($year_arr as $yearSelected) {
                            $same_line = false;
                            foreach($sem_arr as $semesterSelected) {
                                $sqli = "SELECT * FROM tblcmr WHERE ProjectID=:ProjectID AND YearSubmission=:YearSubmission AND Semester=:Semester";
                                $stmt=$db_con->prepare($sqli);
                                $stmt->bindParam(':ProjectID', $projectID, PDO::PARAM_STR);
                                $stmt->bindParam(':YearSubmission', $yearSelected, PDO::PARAM_STR);
