...
var eccfile = document.getElementById("eccfile");
...
newproject_form.addEventListener("submit", function (event) {
  ...
  data.append("eccfile", eccfile.files[0]);
...
function initMap() {
  ...
  google.maps.event.addListener(marker, "dragend", function () {
    ...
    document.getElementById("map-latlng").innerHTML =
      dynamic_address.latitude + ", " + dynamic_address.longitude;
  ...
  geocoder.geocode(
    ...
    function (res, state) {
      ...
      if (state == google.maps.GeocoderStatus.OK) {
        ...
        if (res[1]) {
          ...
          for (var r = 0, r1 = res.length; r < r1; r += 1) {
            ...
            if (!city && result.types[0] === "locality") {
              ...
              for (
                x = 0;
                x < Object.keys(result.address_components[1]).length - 1;
                x++
              ) {
              ...
              !street &&
              result.types[0] === "administrative_area_level_5"
            ) {
        ...
        document.getElementById("map-place").innerHTML =
          '<span class="text-danger">Failed to determine location address!</span>';
      }
    }
  );
}
