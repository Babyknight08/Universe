...
var Toast = Swal.mixin({
  ...
  upload_excel.addEventListener('change', function() {
    if(upload_excel.value.length==0) {
      ...
      if (upload_excel.files[0].type!=='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') {
        span_prompt.classList.remove('text-success')
        span_prompt.classList.add('text-danger')
        ...
        span_size.innerHTML = (upload_excel.files[0].size / (1024*1024)).toFixed(2)
      }
    }
  })
  ...
  $('#modal-delete').on('show.bs.modal', function(event) {
    ...
    document.getElementById('modal-delete-prompt').innerHTML = 'Are you sure you want to delete <b>' + pname + '</b> and all its related files?'
    // console.log(pname)
  })
  ...
  button_delete.addEventListener('click', function() {
    ...
    request.onload = function() {
      ...
      if(response.message == 'success') {
        ...
        setTimeout(function() { $("#modal-delete-prompt").fadeOut(1500); }, 3000)
      }
    }
